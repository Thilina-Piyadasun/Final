 ...
 */
public class PatrolBoundryWorking implements Serializable {
            ...
            for(int i=0;i<leaderList.size();i++){
                ...
                for(long l:neighbours) {
                    ...
                    if(centroidBean!=null) {
                        ...
                        if (distance < min) {
                            follwer = centroidBean.getBlockID();
                            min = distance;
                        }
                    }
                }
                ...
                if (follwer != -1) {
                    leaderBean.addFollower(follwersMap.get(follwer).getBlockID());
                    leaderBean.incrementLeaderWork(follwersMap.get(follwer).getWork() + min);
                    addWork(min);
                    follwersMap.remove(follwer);
                }
            }
    ...
    public void findPatrolBoundries2(){
        ...
        while(follwers.size()>=1) {
            for (int j = follwers.size() - 1; j >= 0; j--) {
                ...
                // for(BlockCentroidBean centroidBean:follwers){
                ...
                for (int i = 0; i < leaderList.size(); i++) {
                    ...
                    if (neighbours.contains(centroidBean.getBlockID())) {
                        ...
                        int threashold=(150*toalWork)/(boundries*100);
                        if(leaderBean.getLeaderWork() < threashold) {
                            if (distance < min) {
                                leader = i;
                                min = distance;
                            }
